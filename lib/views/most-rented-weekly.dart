...
class MostWeekly extends StatefulWidget {
  const MostWeekly({Key key}) : super(key: key);

  @override
  _MostWeeklyState createState() => _MostWeeklyState();
}

class _MostWeeklyState extends State<MostWeekly> {
  int position = 0;
  ...
  Widget build(BuildContext context) {
    // init data
    films = Provider.of<SakilaProvider>(context, listen: true).getMostRentedWeekly(0);
  ...
  Widget filmsGrid(List<Film> films){
    return Padding(
      ...
      child: Column(
        children: [
          Expanded(flex: 1, child: Text("Most Rented of the Week", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 28.0, color: Colors.pink), textAlign: TextAlign.left,)),
          Expanded(
            flex: 12,
